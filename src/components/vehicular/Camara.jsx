...
const Camara = ({ onCapture, fotosRequeridas }) => {
  ...
  const capture = () => {
    ...
    // Aquí recortamos la foto a horizontal (16:9) aunque agarres el cel vertical
    const img = new Image();
    img.src = imageSrc;
    img.onload = () => {
      ...
      canvas.height = Math.round(img.width * (9/16)); // Forzamos la altura panorámica
      
      if (img.height > canvas.height) {
        // Recortamos exactamente el centro (lo que estabas viendo en pantalla)
        const offsetY = (img.height - canvas.height) / 2;
        ctx.drawImage(img, 0, offsetY, img.width, canvas.height, 0, 0, canvas.width, canvas.height);
      } else {
        canvas.height = img.height;
        ctx.drawImage(img, 0, 0);
      }
      
      const fotoRecortada = canvas.toDataURL('image/jpeg', 0.9);

      const nuevasCapturas = { ...capturas, [fotoActiva]: fotoRecortada };
      setCapturas(nuevasCapturas);
          ...
          <div className="camera-preview" style={{ position: 'relative', background: '#000', borderRadius: '8px', overflow: 'hidden' }}>
            ...
            {capturas[fotoActiva] ? (
              ...
              <Webcam
                ...
                // CSS: Fuerza la pantalla a verse en horizontal cortando el excedente
