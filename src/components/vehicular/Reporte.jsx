...
//   const generarID = () => {
//     // 1. Obtenemos el prefijo de la zona GPS
...
//     // 2. Obtenemos el Área del formulario y la cruzamos con el diccionario
//     const areaSeleccionada = datos.form?.area || "Otros";
//     const areaAbbr = ABREVIATURAS[areaSeleccionada] || "OTROS"; 

//     // 3. Generamos la fecha y hora
//     const now = new Date();
//     const timestamp = `${String(now.getDate()).padStart(2,'0')}${String(now.getMonth()+1).padStart(2,'0')}${now.getFullYear()}${String(now.getHours()).padStart(2,'0')}${String(now.getMinutes()).padStart(2,'0')}${String(now.getSeconds()).padStart(2,'0')}`;
    
//     // 4. Devolvemos la segunda parte del ID (ej: RH_SW00026032026123549)
//     return `${areaAbbr}_${codigo}${timestamp}`;
//   };
...
//         historicoFotos[semanaActual].idDocumento = `Vehicular_${ID_GENERADO}`;
//       }
...
//       doc.setProperties({ title: `Vehicular_${ID_GENERADO}`, keywords: estadoParaGuardar });
...
//           doc.setFont('helvetica', 'normal'); doc.text(`Vehicular_${f.idDocumentoPrevio}`, margin + 38, yPos);
//           yPos += 5;
//       } else {
//           yPos += 2;
//       }
...
//       yPos += 1;
//       if (datos.firma) doc.addImage(datos.firma, 'PNG', centerX1 - 15, yPos, 30, 10); 
//       yPos += 10;
...
//       doc.text(`RPE: ${f.rpe || ''}`, centerX1 - 35, yPos + 3);
...
//             const idDoc = fotoData.idDocumento || (semanaNum === semanaActual ? `Vehicular_${ID_GENERADO}` : 'Desconocido');
//             doc.setFont('helvetica','bold');
...
//         doc.text(`ID: Vehicular_${ID_GENERADO} - Pág ${i}/${totalPages}`, pageWidth / 2, pageHeight - 4, { align: 'center' });
//       }

//       // LA MAGIA ESTÁ AQUÍ: El archivo se guarda exactamente con el formato Vehicular_RH_SW000...
//       doc.save(`Vehicular_${ID_GENERADO}.pdf`);
...
//       <p className="text-gray-600 mb-6">El documento vehicular con las semanas llenadas, observaciones y evidencias fotográficas recientes está listo.</p>
...
//       >
//          {enviando ? '⏳ Generando documento...' : '📄 Descargar Vehicular'}
//       </button>
//     </div>
//   );
// };
...
//       // CORRECCIÓN LOGOS: SUTERM ahora es 20x10 (Rectangular y proporcionado)
//       if (imgSuterm) doc.addImage(imgSuterm, 'PNG', margin, yPos, 20, 10);
//       if (imgCfe) doc.addImage(imgCfe, 'PNG', pageWidth - margin - 28, yPos + 2, 28, 10);
...
//       yPos += 1;
//       // CORRECCIÓN FIRMA: Matemáticas para no estirar la firma
//       if (datos.firma) {
//         const firmaProps = doc.getImageProperties(datos.firma);
//         const maxFirmaW = 30;
//         const maxFirmaH = 10;
//         let fW = maxFirmaW;
//         let fH = (firmaProps.height * maxFirmaW) / firmaProps.width;
        
//         if (fH > maxFirmaH) {
//            fH = maxFirmaH;
//            fW = (firmaProps.width * fH) / firmaProps.height;
//         }
        
//         // Centrar la firma en el espacio disponible
//         const fOffsetX = (centerX1 - 15) + (maxFirmaW - fW) / 2;
//         const fOffsetY = yPos + (maxFirmaH - fH) / 2;
        
//         doc.addImage(datos.firma, 'PNG', fOffsetX, fOffsetY, fW, fH);
//       }
//       yPos += 10;
//       doc.line(centerX1 - 35, yPos, centerX1 + 35, yPos);
//       doc.line(centerX2 - 35, yPos, centerX2 + 35, yPos);
...
//               // CORRECCIÓN FOTOS: Cálculo matemático para NUNCA estirar las fotos
//               const imgProps = doc.getImageProperties(imgData);
//               const maxBoxW = 125;
//               const maxBoxH = 65;
              
//               // Primero calculamos el alto ideal si el ancho es 125
//               let finalW = maxBoxW;
//               let finalH = (imgProps.height * maxBoxW) / imgProps.width;
              
//               // Si el alto se pasa de 65, lo achicamos proporcionalmente
//               if (finalH > maxBoxH) {
//                  finalH = maxBoxH;
//                  finalW = (imgProps.width * finalH) / imgProps.height;
//               }
              
//               // Lo centramos horizontalmente en el espacio asignado
//               const offsetX = xPos + (maxBoxW - finalW) / 2;
//               doc.addImage(imgData, 'JPEG', offsetX, yPosF + 3, finalW, finalH);
//             });
//           } else {
//             if (!paginaHistoricoAgregada) {
//               doc.addPage();
...
//       // ==========================================
//       // MAGIA: PÁGINA DE ANEXOS CON OBSERVACIONES
//       // ==========================================
//       [1, 2, 3, 4].forEach((semanaNum) => {
//         const fotoData = historicoFotos[semanaNum];
//         if (fotoData && (fotoData.hora || fotoData.capturas)) {
...
//             doc.setFontSize(10); doc.setFont('helvetica','bold');
//             doc.text("Observaciones de la semana:", margin, 54);

//             // Extraemos las observaciones de la semana desde el checklist
//             const obsSemana = historicoSemanas[semanaNum]?._observaciones || {};
//             const listaObs = Object.entries(obsSemana).map(([item, texto]) => `${item}: ${texto}`);

//             let textoObservaciones = listaObs.length > 0 
//                 ? listaObs.join("  |  ") 
//                 : "No se registraron observaciones adicionales durante la inspección de esta semana.";

//             doc.setFont('helvetica','normal');
//             doc.setTextColor(200, 0, 0); // TEXTO ROJO

//             // Dividimos el texto rojo para que no se salga de la hoja si escriben mucho
//             const lineasObs = doc.splitTextToSize(textoObservaciones, pageWidth - (margin * 2));
//             doc.text(lineasObs, margin, 60);

//             doc.setTextColor(0, 0, 0); // Regresamos al texto negro para las fotos
            
//             // Empujamos el inicio de las fotos hacia abajo, dependiendo de cuántas líneas ocupó la observación
//             let currentY = 62 + (lineasObs.length * 5); 
...
//               const imgProps = doc.getImageProperties(imgData);
//               const maxBoxW = 125;
//               const maxBoxH = 60; // Lo redujimos a 60 para que no choque con las observaciones de arriba
...
//             });
//           } else {
//             // (Esta es la sección por si algún día necesitas anexar una semana sin fotos, se mantiene igual)
//             if (!paginaHistoricoAgregada) {
//               doc.addPage();
//               doc.setFontSize(14); doc.setFont('helvetica','bold');
//               doc.text(`REGISTRO HISTÓRICO DE EVIDENCIAS`, margin, 20);
//               paginaHistoricoAgregada = true;
//             }
//           }
//         }
...
const Reporte = ({ datos, historicoSemanas, historicoFotos, showNotification }) => {
  ...
  const generarPDF = async () => {
    try {
      ...
      // ==========================================
      // CORRECCIÓN: HOJAS DE ANEXO (CON O SIN FOTOS)
      // ==========================================
      [1, 2, 3, 4].forEach((semanaNum) => {
        const fotoData = historicoFotos[semanaNum];
        // Revisamos si esta semana tiene CUALQUIER tipo de dato
        if (fotoData && (fotoData.hora || fotoData.direccion || (fotoData.capturas && Object.keys(fotoData.capturas).length > 0))) {
          
          const tieneImagenes = fotoData.capturas && Object.keys(fotoData.capturas).length > 0;
          
          // Extraemos las observaciones de la semana
          const obsSemana = historicoSemanas[semanaNum]?._observaciones || {};
          const listaObs = Object.entries(obsSemana).map(([item, texto]) => `${item}: ${texto}`);
          let textoObservaciones = listaObs.length > 0 
              ? listaObs.join("  |  ") 
          ...
          if (tieneImagenes) {
            // ... (Si tiene imágenes, imprime la hoja normal)
            doc.addPage();
            doc.setFontSize(14); doc.setFont('helvetica','bold');
            ...
            // === ESTA ES LA CORRECCIÓN: HOJA HISTÓRICA SIN FOTOS PERO CON TEXTO ===
            if (!paginaHistoricoAgregada) {
              doc.addPage();
              doc.setFontSize(14); doc.setFont('helvetica','bold');
              doc.setTextColor(0,0,0);
            ...
            if (currentYTextOnly > pageHeight - 50) {
              doc.addPage();
              currentYTextOnly = 20;
            }

            doc.setFontSize(11); doc.setFont('helvetica','bold');
            doc.text(`DATOS DE LA SEMANA ${semanaNum}`, margin, currentYTextOnly); currentYTextOnly += 6;
            ...
            // IMPRIMIR LAS OBSERVACIONES ROJAS AUNQUE NO HAYA FOTOS
            doc.setFontSize(10); doc.setFont('helvetica','bold');
            doc.text("Observaciones de la semana:", margin, currentYTextOnly); currentYTextOnly += 5;
            doc.setFont('helvetica','normal'); doc.setTextColor(200, 0, 0); 
            const lineasObs = doc.splitTextToSize(textoObservaciones, pageWidth - (margin * 2));
            doc.text(lineasObs, margin, currentYTextOnly);
            currentYTextOnly += (lineasObs.length * 5);
            doc.setTextColor(0, 0, 0);
